...
namespace PS {
    ...
    auto Log(RC::File::StringViewType content, FmtArgs... fmt_args) -> void
    {
        ...
        {
            auto formatted_log = std::format(STR("[DawnSchema] [error] {}"), content);
            RC::Output::send<optional_arg>(formatted_log, fmt_args...);
        }
        else if (optional_arg == RC::LogLevel::Warning)
        {
            auto formatted_log = std::format(STR("[DawnSchema] [warning] {}"), content);
            RC::Output::send<optional_arg>(formatted_log, fmt_args...);
        }
        else if (optional_arg == RC::LogLevel::Verbose)
        {
            ...
            auto formatted_log = std::format(STR("[DawnSchema] [debug] {}"), content);
            RC::Output::send<optional_arg>(formatted_log, fmt_args...);
        }
        else
        {
            auto formatted_log = std::format(STR("[DawnSchema] {}"), content);
            RC::Output::send<optional_arg>(formatted_log, fmt_args...);
        }
    }
}