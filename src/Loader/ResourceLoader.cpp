...
namespace Schema {
    ResourceLoader::ResourceLoader() : ModLoaderBase("resources")
    {
    }
    ...
    void ResourceLoader::OnLoad(const fs::path& loaderPath, const RC::StringType& modName, const EEngineLifecyclePhase& engineLifecyclePhase)
    {
    ...
    void ResourceLoader::OnAutoReload(const RC::StringType& modName, const fs::path& modFilePath)
    {
        UnregisterResourceAssetByFilePath(modName, modFilePath);
        LoadImage(modName, modFilePath);
    }

    bool ResourceLoader::CanInitialize(const EEngineLifecyclePhase& engineLifecyclePhase)
    {
        if (engineLifecyclePhase == EEngineLifecyclePhase::GameInstanceInit)
        {
            return true;
    ...
    bool ResourceLoader::OnInitialize()
    {
        return true;
    }

    void ResourceLoader::RegisterResourceAsset(const RC::StringType& modName, UObject* resource)
    {
    ...
    void ResourceLoader::UnregisterResourceAsset(UObject* resource)
    {
    ...
    void ResourceLoader::UnregisterResourceAssetByFilePath(const RC::StringType& modName, const fs::path& modFilePath)
    {
        ...
        std::erase_if(loadedResources, [&](UObject* loadedResource) {
        ...
        // Manually calling GC here. It's fine because UnregisterResourceAssets only happens when auto-reloading or reloading DawnSchema mods.
        UECustom::UKismetSystemLibrary::CollectGarbage();
    }

    void ResourceLoader::UnregisterResourceAssets(const RC::StringType& modName)
    {
        auto loadedResourcesIt = m_loadedResourcesMap.find(modName);
        if (loadedResourcesIt != m_loadedResourcesMap.end())
        {
    ...
    void ResourceLoader::UnregisterResourceAssets()
    {
    ...
    void ResourceLoader::LoadImages(const RC::StringType& modName, const fs::path& resourcesPath)
    {
    ...
    void ResourceLoader::LoadImage(const RC::StringType& modName, const fs::path& imagePath)
    {
        ...
        auto packagePath = std::format(TEXT("DawnSchema/Resources/{}/{}"), modName, imageName);
        newTexture->Rename(packagePath.c_str()); // becomes "/Engine/Transient.DawnSchema/Resources/modname/resourcename"
